...
备选方案中，DTW \cite{sakoe1978} 时间复杂度 $O(N^2)$ 且精度受限于采样间隔（0.1\,s）；互相关法需均匀采样且对非周期信号峰值定位不稳定。因此选用三次样条插值 + 有界一维优化：样条将离散轨迹延拓为 $C^2$ 连续函数，搜索精度可达 $10^{-6}$\,s，计算量 $O(N\log(1/\epsilon))$。
...
分步法（先估 $\Delta\tau$ 再估空间偏差）的问题在于 $\Delta\tau$ 的估计偏差会系统性地传入空间偏差的估计。将三参数放入同一目标函数 $J(\Delta\tau, \Delta x, \Delta y)$ 联合求解（网格预搜索 + Nelder-Mead 精细化），一步到位绕开逐步估计的累积偏差。融合阶段，由于观测模型为线性（$H=[I_2, 0]$），标准卡尔曼滤波 \cite{welch2006} 已是最优估计器，不需引入 EKF 或 PF。两路按时间戳排序后异步更新 \cite{hall1997}。
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[node distance=0.7cm and 1.8cm,
        ...
        \node[font=\tiny, above=0.1cm of kf] {$q_a=5$\,m/s$^2$};
    \end{tikzpicture}
...
\textbf{方法选型对比}：混合整数线性规划（MILP）变量规模过大（$10^5$ 量级约束），CBC/Gurobi 在此规模下计算耗时超出限制。采用三层求解策略：（1）SG 滤波（$w=151$，3 阶）平滑轨迹后贪心启发式快速给出基线解；（2）加权区间调度 DP 尝试改进；（3）CP-SAT 约束规划 \cite{ortools2024} 对稀疏化候选集建 0-1 模型精确求解。CP-SAT 在约 20\,s 内证明最优（591 个决策变量），射击数从贪心的 8 提升至 10。灵敏度分析确认 $w = 151$ 为当前轨迹的最优工作点。
